...
export default function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  const isImmersive = pathname === "/todo" || pathname === "/ai" || pathname === "/settings" || pathname.startsWith("/learn/");
  ...
  return (
    ...
    <HapticRouterProvider>
      ...
      <main className={`min-h-screen ${isImmersive ? "px-0 lg:pl-[360px] lg:pr-8 pt-0 pb-0 max-w-full" : "px-4 lg:pl-[360px] lg:pr-8 pt-2 lg:pt-4 pb-44 lg:pb-12 max-w-[1400px]"}`}>
        {!isImmersive && <TitleBar />}
        {children}
      </main>
      {!isImmersive && (
        <div 
          className="lg:hidden fixed bottom-0 left-0 w-full h-48 pointer-events-none z-[40] backdrop-blur-lg"

...
import { ArrowRight, Atom, Dna, GraduationCap, Phone, Target } from "lucide-react";
...
export default function Onboarding() {
  ...
  const finish = async (ex: ExamType, s: Stream | null, v: VariantType | null) => {
    if (!grade) return;
    setBusy(true);
    setErr("");
    try {
      await completeOnboarding({ phone: phone.trim(), grade, stream: s, exam: ex, variant: v });
      vibrate(30);
      router.replace("/home");
    } catch (e) {
  ...
  const steps = [
    ...
    },
    {
      ...
      sub: "Your daily plan will be tailored for this exam.",
      body: (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
          {([
            { id: "MHT_CET" as ExamType, label: "MHT-CET", desc: "Maharashtra State" },
            { id: "JEE" as ExamType, label: "JEE", desc: "Main & Advanced" },
            { id: "NEET" as ExamType, label: "NEET", desc: "Medical UG" }
          ]).map((ex) => (
            <motion.button
              key={ex.id}
              whileTap={{ scale: 0.95 }}
              onClick={() => {
                ...
                setExam(ex.id);
                if (ex.id === "NEET") {
                  void finish("NEET", null, null);
                } else {
                  setStep(3);
                }
              }}
              className="bg-white/20 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-3xl p-6 transition-colors text-left hover:bg-white/40 dark:hover:bg-white/10"
            >
              <p className="font-sora text-2xl font-black text-neutral-900 dark:text-white">{ex.label}</p>
              <p className="font-geist text-sm text-black dark:text-neutral-400 mt-2">{ex.desc}</p>
            </motion.button>
          ))}
        </div>
      ),
    },
    {
      ...
      title: exam === "JEE" ? "Select JEE Target" : "Pick your stream",
      sub: "You'll only ever see content for your selection.",
      body: (
        <div className="grid grid-cols-2 gap-4 mt-6">
          {exam === "MHT_CET" && ([
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              disabled={busy}
              onClick={() => { vibrate(15); setStream(s); void finish("MHT_CET", s, null); }}
          ...
          {exam === "JEE" && ([
            { v: "MAIN" as VariantType, label: "JEE Main", sub: "NITs / IIITs" },
            { v: "ADVANCED" as VariantType, label: "JEE Advanced", sub: "IITs" }
          ]).map(({ v, label, sub }) => (
            <motion.button
              key={v}
              whileTap={{ scale: 0.95 }}
              disabled={busy}
              onClick={() => { vibrate(15); setVariant(v); void finish("JEE", null, v); }}
              className={`bg-white/20 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-3xl p-6 transition-all text-left ${variant === v && busy ? "border-purple-500 shadow-[0_0_20px_rgba(168,85,247,0.3)]" : "hover:bg-white/40 dark:hover:bg-white/10"}`}
            >
              <span className="text-purple-600 dark:text-purple-400"><Target size={24} /></span>
              <p className="font-sora text-2xl font-black text-neutral-900 dark:text-white mt-4 tracking-tight">{label}</p>
              <p className="font-geist text-xs font-semibold text-black dark:text-neutral-400 mt-1">{sub}</p>
            </motion.button>
          ))}
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 pt-[calc(env(safe-area-inset-top,3rem)+2rem)] pb-[calc(env(safe-area-inset-bottom,2rem)+2rem)] md:p-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl flex flex-col md:flex-row bg-white/20 dark:bg-white/5 backdrop-blur-lg border border-white/30 dark:border-white/10 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl relative z-10 overflow-hidden min-h-[600px]">
        ...
        <div className="md:w-5/12 h-56 md:h-auto relative flex flex-col justify-end p-8 md:p-10 bg-gradient-to-br from-purple-900/50 to-blue-900/50">
          <img src="/onboarding-bg.jpg" className="absolute inset-0 w-full h-full object-cover opacity-60 mix-blend-overlay" alt="Abstract" />
        ...
        <div className="flex-1 p-8 md:p-16 flex flex-col">
          ...
          <div className="flex gap-2 mb-12">
            {[0, 1, 2].map((_, i) => (
              <motion.div 
                key={i} 
                className={`h-2 rounded-full ${i <= Math.min(step, 2) ? 'bg-gradient-to-r from-purple-600 to-blue-600' : 'bg-black/10 dark:bg-white/10'}`} 
                animate={{ flex: i === Math.min(step, 2) ? 3 : 1 }} 
                transition={spring} 
              />
            ))}
          </div>

...
import {
  updateContent, updateTest, updateVideo,
} from "@/lib/db";
import {
  type Stream, type Weightage, type ContentDoc, type TestDoc, type VideoDoc,
  extractDriveId, extractYouTubeId
} from "@/lib/types";
import { useToast } from "@/components/Toast";

const STREAMS: Stream[] = ["PCM", "PCB", "PCMB"];
const TYPES = ["Notes PDF", "DPP", "Formula Sheet", "PYQ Pack", "External Link", "Video"];
    ...
    <motion.button
      ...
      className={`rounded-full px-4 py-2 font-geist text-xs font-bold transition-all border ${
        active 
          ...
          : "bg-black/5 dark:bg-white/5 border-black/10 dark:border-white/10 text-neutral-600 dark:text-neutral-400 hover:bg-black/10 dark:hover:bg-white/10 hover:text-neutral-900 dark:hover:text-white"
      }`}
    >
      {label}
    </motion.button>
...
export default function ContentEditPage() {
  ...
  const initMode = searchParams.get("mode") as "pdf" | "test" | "video" | null;
  ...
  useEffect(() => {
    if (!id || !initMode) return;
    const fetchDoc = async () => {
      try {
        const colName = initMode === "pdf" ? "content" : initMode === "video" ? "videos" : "tests";
        const snap = await getDoc(doc(fbDb(), colName, id));
        if (snap.exists()) {
  ...
  const urlOk = mode === "pdf" ? (type === "External Link" ? true : !!extractDriveId(url)) : mode === "video" ? !!extractYouTubeId(url) : url.includes("docs.google.com/forms");
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center"><div className="w-8 h-8 rounded-full border-2 border-white/50 border-t-transparent animate-spin" /></div>;
  }
  ...
  return (
    <motion.div 
      ...
      exit={{ opacity: 0, y: 20 }}
      className="w-full max-w-7xl mx-auto flex flex-col gap-6 pt-2 pb-24 lg:pb-12"
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-sora text-headline-xl">Edit Content</h1>
        </div>
        <motion.button
          whileTap={{ scale: 0.95 }}
          onClick={handleSave}

...
export default function Builder() {
  ...
  const [bStreams, setBStreams] = useState<Stream[]>(["PCM", "PCB", "PCMB"]);
  ...
  const [nStreams, setNStreams] = useState<Stream[]>(["PCM", "PCB", "PCMB"]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-6 pt-2 pb-20 lg:pb-12">
            ...
            <AnimatePresence>
              {blocks.filter(id => !hidden.includes(id)).map((id) => (
                <motion.div key={id} layout transition={{ type: "spring", stiffness: 380, damping: 30 }} className="mb-4 px-6 relative">
                  ...
                  {id === "carousel" && (
                    <div onClick={(e) => { e.preventDefault(); toast.info(`Link would open: ${addingNew ? bCta : (banners[0]?.cta || '#')}`); }} className="rounded-[1.5rem] h-40 p-5 flex flex-col justify-end relative overflow-hidden cursor-pointer" style={{ background: "linear-gradient(120deg, #9333ea, #3b82f6)" }}>

...
import { getCountFromServer, query, where, orderBy, limit, getDocs } from "firebase/firestore";
...
export default function AdminDashboard() {
  ...
  const [counts, setCounts] = useState({ content: 0, tests: 0, banners: 0, users: 0, active: 0 });
  const [seeding, setSeeding] = useState(false);
  const [expandingCard, setExpandingCard] = useState<string | null>(null);
  const [aiLimit, setAiLimit] = useState(1000);
  const [topUsers, setTopUsers] = useState<UserDoc[]>([]);
  ...
  useEffect(() => {
    ...
    // Use getCountFromServer instead of streaming entire collections
    const fetchStats = async () => {
      try {
        const [cSnap, tSnap, bSnap, uSnap, aSnap] = await Promise.all([
          getCountFromServer(col.content()),
          getCountFromServer(col.tests()),
          getCountFromServer(col.banners()),
          getCountFromServer(col.users()),
          getCountFromServer(query(col.users(), where("lastActiveDate", "==", new Date().toISOString().slice(0, 10))))
        ]);
        if (mounted) {
          setCounts({ 
            content: cSnap.data().count, 
            tests: tSnap.data().count, 
            banners: bSnap.data().count, 
    ...
    const fetchTop = async () => {
      try {
        const q = query(col.users(), orderBy("coins", "desc"), limit(3));
        const snap = await getDocs(q);
        if (mounted) setTopUsers(snap.docs.map((d) => snapTo<UserDoc>(d)));
      } catch (e) {
        console.warn("Top users fetch failed", e);
      }
    };
  ...
  const initialize = async () => {
    ...
    try {
      ...
      toast.success("🚀 SKCTI OS Initialized successfully!");
      await logAudit(fbUser.email, "Initialized app config & claimed admin");
    } catch (error) {
      console.error("Initialization failed:", error);
      toast.error("Error: Check Firestore Rules and your connection.");
    }
    setSeeding(false);
  };

  const kpis = [
    { id: "card-users", Icon: Users, label: "Students", val: counts.users, sub: `${counts.active} active today`, route: "/admin/users" },

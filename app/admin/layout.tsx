...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const { ready, fbUser, isAdmin, configLoaded, profile, logout } = useStore();
      ...
      <div className="pt-5 border-t border-black/10 dark:border-white/10 mt-auto shrink-0">
        <Link href="/home" className="block" onClick={() => { vibrate(10); setMobileMenuOpen(false); }}>
          <div className="w-full flex items-center gap-3 p-3 pl-4 bg-black/5 dark:bg-white/5 backdrop-blur-md border border-black/10 dark:border-white/10 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-all shadow-md cursor-pointer group">
            ...
            <button
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); vibrate(15); void logout().then(() => router.replace("/")); }}
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row relative">
      ...
      {/* Floating Mobile Header */}
        <header className="lg:hidden fixed top-0 inset-x-0 z-50 p-4 pt-[calc(env(safe-area-inset-top,3rem)+1.5rem)] flex justify-between items-start pointer-events-none">
      ...
      <main className={`flex-1 min-w-0 overflow-y-auto relative lg:ml-[360px] px-4 md:px-8 lg:px-12 pt-[calc(env(safe-area-inset-top,3rem)+6rem)] lg:pt-10 lg:py-10`}>{children}</main>
            ...
            {fabOpen && (
              <>
                ...
                <motion.div initial={{ opacity: 0, y: 10, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.8 }}>
                  <button onClick={() => { triggerHaptic(); setFabOpen(false); toast.info("Add Notice (Coming Soon)"); }} className="flex items-center gap-3 group">

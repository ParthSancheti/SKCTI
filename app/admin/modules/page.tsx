...
import { createModule, createDefaultModule, deleteModule, logAudit, updateModule } from "@/lib/db";
...
export default function ModulesHub() {
  ...
  const remove = async (m: ModuleDoc) => {
    if (m.id.startsWith("default_")) {
      alert("This is a system default module. You can edit it to override it, but you cannot delete it directly.");
      return;
    }
  ...
  const seedDefaultModules = async () => {
    ...
    const defaults = [
      ...
      { id: "default_biology", name: "Biology", streams: ["PCB"], imageUrl: "https://images.unsplash.com/photo-1530213786676-4c72478563a6?q=80&w=1000&auto=format&fit=crop" }
    ];
    setSaving(true);
    try {
      ...
      console.error(err);
      alert("Failed to restore modules.");
    }
    setSaving(false);
  };

...
const CONFIG: Record<ToastKind, { Icon: typeof CheckCircle2; accent: string; ms: number }> = {
  ...
  error:   { Icon: XCircle,        accent: "text-red-500",     ms: 5000 },
        ...
        {items.map(({ id, kind, message }) => {
          ...
          return (
            ...
            >
              ...
              <p className="font-hanken text-body-md flex-1 leading-snug text-neutral-900 dark:text-white">
                {message}
              </p>
              <button
    ...
    <AnimatePresence>
      {state.open && (
        ...
        >
          ...
          >
            <p className="font-hanken text-body-lg text-neutral-900 dark:text-white">{state.message}</p>

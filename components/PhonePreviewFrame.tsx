...
export function LiveChapterCard({
  ...
  return (
    <div className="bg-surface-container backdrop-blur-xl border border-white/10 rounded-[1.5rem] shadow-lg overflow-hidden transition-all">
      <div className="p-5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="font-sora font-semibold text-on-surface truncate">{title || "Untitled chapter"}</h3>
            <p className="font-geist text-xs text-on-surface-variant truncate">{subject || "Subject"} · {stream || "Stream"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-wrap gap-2 mb-1">
            <span className="flex items-center gap-1.5 bg-error-container border border-red-500/20 text-error px-2.5 py-1 rounded-full font-geist text-[10px] font-bold tracking-wider uppercase shadow-[0_0_15px_rgba(239,68,68,0.2)]">
              <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse shadow-[0_0_5px_rgba(239,68,68,0.8)]" />
              Live
            </span>
            <span className="bg-surface-container border border-outline-variant text-neutral-700 dark:text-white/70 px-2.5 py-1 rounded-full font-geist text-[10px] font-bold tracking-wider uppercase">
              {stream || "Stream"}
            </span>
          </div>
        </div>
        ...
        <div className="min-w-0">
          <h3 className="font-sora font-bold text-lg leading-tight text-on-surface mb-1">{title || "Untitled Test"}</h3>
          <p className="font-geist text-xs text-on-surface-variant">{subject || "Subject"}</p>
        </div>

        <div className="grid grid-cols-2 gap-3 py-3 border-y border-outline-variant">
          <div className="flex flex-col">
            <span className="font-geist text-[10px] uppercase tracking-wider text-on-surface-variant mb-0.5">Duration</span>
            <span className="font-sora font-semibold text-on-surface text-sm">{duration || "0"} mins</span>
          </div>
          <div className="flex flex-col">
            <span className="font-geist text-[10px] uppercase tracking-wider text-on-surface-variant mb-0.5">Total Marks</span>
            <span className="font-sora font-semibold text-on-surface text-sm">{marks || "0"}</span>
          </div>
        </div>

...
export default function AuditLog() {
  ...
  return (
    <GlassCard className="p-8">
      ...
      {rows.length === 0 ? (
        <p className="font-hanken text-body-md text-on-surface-variant">No admin actions yet — publish something and it lands here.</p>
      ) : (
        <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-black/10 dark:[&::-webkit-scrollbar-thumb]:bg-white/10">
          {rows.map((r) => (
            <div key={r.id} className="flex items-start gap-3 border-t border-outline-variant pt-4 first:border-0 first:pt-0">
              <span className="w-2 h-2 rounded-full bg-purple-600 dark:bg-purple-400 mt-2 shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="font-hanken text-body-md text-on-surface">{r.action}</p>
                <p className="font-geist text-label-sm text-on-surface-variant mt-0.5">
